...
def get_pitching_lastxgames(pitcher_id, hit_rank: pd.DataFrame, num_of_games=5):
    ...
    if games.__len__() >= num_of_games:
        ...
        aggregate_stats['rank:Runs'] = {
            get_hit_rank_position(hit_rank, game.opponent['name'], **runs_input): game.stat.runs for game in
            games}
        aggregate_stats['SO'] = median(tuple(game.stat.strikeouts for game in games))
        aggregate_stats['rank:SOs'] = {get_hit_rank_position(hit_rank, game.opponent['name']): game.stat.strikeouts for
                                       game in games}
...
def get_pitch_ranking():
    # TODO get different rankings for ome and away
...
def pitcher_table(same_day=True):
    ...
    odds = ScraperScripts.get_category_odds(ScraperScripts.Sports.mlb, ScraperScripts.Categories.pitchers,
                                            ScraperScripts.SubCategories.SOs,
    ...
    for game in schedule:
        ...
        if all(vars(game_data.gamedata.probablepitchers).values()):
            ...
            if away_pitch_er:=pitcher_df.at[unidecode(game_data.gamedata.probablepitchers.away.fullname), 'runs'] is not np.nan:
                run_pot['home_pot'] = mean([hit_rank.loc[run_pot['home'], 'R'],pitch_rank.loc[run_pot['away'], 'ER']]+[away_pitch_er])
            else:
                run_pot['home_pot'] = mean([hit_rank.loc[run_pot['home'], 'R'], pitch_rank.loc[run_pot['away'], 'ER']])
            if home_pitch_er := pitcher_df.at[unidecode(game_data.gamedata.probablepitchers.home.fullname), 'runs'] is not np.nan:
                run_pot['away_pot'] = mean([hit_rank.loc[run_pot['away'], 'R'],pitch_rank.loc[run_pot['home'], 'ER']]+[home_pitch_er])
            else:
                run_pot['away_pot'] = mean([hit_rank.loc[run_pot['away'], 'R'], pitch_rank.loc[run_pot['home'], 'ER']])
