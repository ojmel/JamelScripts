...
import json, re, string, requests, statsapi, mlbstatsapi, ScraperScripts
...
def pitcher_table(same_day=True):
    ...
    for game in schedule:
        ...
        for side, pitcher in vars(game_data.gamedata.probablepitchers).items():
            ...
            if attrgetter(f"{opp_side}")(game_data.gamedata.probablepitchers) and (pitch_er :=pitcher_df.at[
                                    unidecode(attrgetter(f"{opp_side}.fullname")(game_data.gamedata.probablepitchers)), 'runs']) is not np.nan and pitcher_df.at[
                                    unidecode(attrgetter(f"{opp_side}.fullname")(game_data.gamedata.probablepitchers)), 'IP']>=3.0:
