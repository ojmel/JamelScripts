...
translation={'State': 'St.','International':'Intl.'}
def translate_teams(team:str):
...
def get_conference_stats(off_def:Side,conference):
    ...
    stats=get_url_soup(rf'https://sports.yahoo.com/ncaaf/stats/team/?selectedTable={off_def.value}&leagueStructure=ncaaf.struct.div.{div}.conf.{conf_id}')
...
if __name__=='__main__':
    ...
    try:
        ...
        api_response = api_instance.get_lines(year=2024, week=8)
        games=[game_info for game_info in api_response if game_info.away_conference==game_info.home_conference and game_info.lines]
        games=[{'home':translate_teams(game.home_team),'away':translate_teams(game.away_team),'conf':game.away_conference} for game in games]
        ...
        for game_info in games:
            ...
            if game_info['home'] not in offense.index:
                closest_matches = difflib.get_close_matches(game_info['home'], offense.index.to_list(), n=3, cutoff=0.6)
                home=closest_matches[0]
            else:
                home=game_info['home']

            if game_info['away'] not in offense.index:
                closest_matches = difflib.get_close_matches(game_info['away'], offense.index.to_list(), n=3, cutoff=0.6)
